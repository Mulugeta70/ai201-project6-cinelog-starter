...
tests/test_collection.py (see that file for the conventions being mirrored).
"""
...
def test_add_to_watchlist_defaults_to_private(app, sample_user, sample_film):
    """
    Without an explicit public argument, a new entry should default to
    private. See pr-response.md Comment 4 for the reasoning.
    """
    with app.app_context():
        entry = add_to_watchlist(user_id=sample_user, film_id=sample_film)
...
def test_get_watchlist_returns_newest_first(app, sample_user):
    ...
    (most recently added first), not alphabetically. See Comment 5 in
    pr-response.md for the reasoning.
    """
    with app.app_context():
        from datetime import datetime, timezone, timedelta

        # Titles are chosen so alphabetical order would produce the
        # opposite result, proving the sort is by date, not by title.
        ...
        # Blade Runner was added later, so it should come first even
        # though it's alphabetically after Alien.
